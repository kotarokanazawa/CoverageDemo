...
def choose_frontier_goal_with_candidates(known_map, robot_cell):
    ...
    for y, x, unknown_count in fs[:MAX_EVALUATED_FRONTIER_CANDIDATES]:
        ...
        score = UNKNOWN_GAIN * unknown_count - DIST_GAIN * dist
        cost = dist
        candidates.append((y, x, cost, score, dist, unknown_count))

        if score > best_score:
            best_score = score
            best_goal = goal
            best_path = rebuild_path(came, robot_cell, goal)

    candidates.sort(key=lambda v: v[2])

    return best_goal, best_path, candidates
...
def main():
    ...
    def update(frame):
        ...
        cost_info_text.set_text(
            "candidate number: path length on known free cells [cell]\n"
            "candidate: known free cell adjacent to unknown cells\n"
            "score = unknown_gain * unknown_neighbors - dist_gain * path_length\n"
            "star: candidate with the maximum score"
        )
